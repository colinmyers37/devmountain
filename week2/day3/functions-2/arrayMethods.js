...
let number_even = mixedNumbers.filter((mixedNumber) => mixedNumber % 2 === 0)
...
let postTax = prices.map((price) => price * 1.07)
console.log(postTax)
...
const populationsSum = populations.reduce((each, total) => each + total)
...
let cpOver = monstersInYourPocket.filter((monster_object) => monster_object.CP >= 200)
console.log(cpOver);
...
let withTax = orders.map((ogPrice) => ogPrice.price * ogPrice.tax);
console.log(withTax);
...
const bobsTotal = purchases.filter((onlyBob) => onlyBob.owner === "Bob").reduce((tot, og) => og.price + tot, 0)
